...
void CAPRSWriter::sendIdFrameFixed()
{
	...
	if (m_txFrequency != 0U) {
		...
		::sprintf(desc, "MMDVM Voice (M17) %.5LfMHz %c%.4lfMHz%s%s",
			(long double)(m_txFrequency) / 1000000.0F,
			offset < 0.0F ? '-' : '+',
			::fabs(offset), m_desc.empty() ? "" : ", ", m_desc.c_str());
	} else {
		::sprintf(desc, "MMDVM Voice (M17)%s%s", m_desc.empty() ? "" : ", ", m_desc.c_str());
	}
	...
	char output[500U];
	::sprintf(output, "%s>APDG17,TCPIP*,qAC,%s:!%s%c%c%s%c%c/A=%06.0f%s %s\r\n",
