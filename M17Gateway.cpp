...
static CM17Gateway* gateway = nullptr;
	...
	while (!m_killed) {
		...
		if (ret) {
			...
			if (m_status == M17_STATUS::LINKED) {
				...
				M17NET_STATUS netStatus = m_network->getStatus();
				if (netStatus == M17NET_STATUS::FAILED) {
...
void CM17Gateway::writeCommand(const std::string& command)
{
	if (command.substr(0, 9) == "Reflector") {
		...
		CUtils::dump("Invalid remote command received", (unsigned char*)command.c_str(), (unsigned int)command.size());
	}
}
